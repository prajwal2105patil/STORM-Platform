...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: 'Georgia', serif; color: #1a1a2e; background: white; padding: 40px; max-width: 900px; margin: 0 auto; }
...
<body>
  ...
  try {
    ...
    const filename = `ASRE-Evidence-Report-${id.slice(0, 8)}.pdf`;
    ...
    return NextResponse.json(
      { error: "PDF generation failed — use the HTML report as fallback" },
      { status: 500 }
    );
  }
}
