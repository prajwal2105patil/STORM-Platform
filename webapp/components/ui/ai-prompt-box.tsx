...
// Inject minimal custom scrollbar styles (client-only)
if (typeof document !== "undefined") {
  const id = "__prompt-box-styles";
...
const PromptInputTextarea: React.FC<PromptInputTextareaProps & React.ComponentProps<typeof Textarea>> = ({
  ...
  return (
    <Textarea ref={textareaRef} value={value} onChange={(e) => setValue(e.target.value)}
      onKeyDown={handleKeyDown} className={cn("text-base", className)}
      disabled={disabled} placeholder={placeholder} {...props} />
  );
};

// ── PromptInputActions / PromptInputAction ────────────────────────────────────
...
export const PromptInputBox = React.forwardRef<HTMLDivElement, PromptInputBoxProps>((props, ref) => {
  ...
  const handleRemoveFile = (index: number) => {
    if (files[index] && filePreviews[files[index].name]) setFilePreviews({});
    setFiles([]);
  };
  ...
  const toggleMode = (mode: "search" | "think" | "canvas") => {
    ...
    else if (mode === "think") { setShowThink((p) => !p); setShowSearch(false); }
    else { setShowCanvas((p) => !p); }
  };
