...
interface GlobeProps {
  /**
   * standalone=true (default): renders the globe centered inside a full-screen
   * flex container — matches the original demo layout.
   * standalone=false: renders only the 250×250 globe div, for embedding inside
   * existing layouts without the h-screen wrapper.
   */
  standalone?: boolean;
}

const Globe: React.FC<GlobeProps> = ({ standalone = true }) => {
  const styles = `
  ...
  const globeEl = (
    ...
    >
      {/* Twinkling stars positioned around the globe */}
      <div
